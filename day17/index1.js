...
eachLine(filename, function(line) {
  ...
  console.log(paths);
  let done = false;
  for(let p=0;!done&&p<maxPath;p++) {
    console.log(`Trying loops of length+heuristic of ${p} with ${posPath}`);
...
function expandR(d,p,y,x) {
  if(x===tarx||(!/R/.test(p.substr(-1))&&x>=(tarx-minD+1))||/RRRRRRRRRR/.test(p.substr(-10))||/L/.test(p.substr(-1))||(p.length>0&&!/R/.test(p.substr(-1))&&(!/UUUU/.test(p.substr(-4))&&!/DDDD/.test(p.substr(-4))))) {
    return;
  }
...
function expandD(d,p,y,x) {
  if(y===tary||(!/D/.test(p.substr(-1))&&y>=(tary-minD+1))||/DDDDDDDDDD/.test(p.substr(-10))||/U/.test(p.substr(-1))||(p.length>0&&!/D/.test(p.substr(-1))&&(!/RRRR/.test(p.substr(-4))&&!/LLLL/.test(p.substr(-4))))) {
    return;
  }
...
function expandL(d,p,y,x) {
  if(x===0||(!/L/.test(p.substr(-1))&&x<=(minD-1))||/LLLLLLLLLL/.test(p.substr(-10))||/R/.test(p.substr(-1))||(p.length>0&&!/L/.test(p.substr(-1))&&(!/UUUU/.test(p.substr(-4))&&!/DDDD/.test(p.substr(-4))))) {
    return;
  }
  let newd = d + island[y][x-1];
...
function expandU(d,p,y,x) {
  if(y===0||(!/U/.test(p.substr(-1))&&y<=(minD-1))||/UUUUUUUUUU/.test(p.substr(-10))||/D/.test(p.substr(-1))||(p.length>0&&!/U/.test(p.substr(-1))&&(!/LLLL/.test(p.substr(-4))&&!/RRRR/.test(p.substr(-4))))) {
    return;
  }
  let newd = d + island[y-1][x];
